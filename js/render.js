...
// Vidéo si dispo, sinon première photo, sinon placeholder — utilisé par les cartes.
function mediaSwatch(p) {
  if (p.video) {
    return `<video class="arrivage-card__swatch" src="${p.video}" muted loop playsinline preload="metadata"></video>`;
  }
  const photos = Array.isArray(p.photos) ? p.photos : [];
  if (photos.length) {
    return `<img class="arrivage-card__swatch" src="${photos[0]}" alt="${p.nom || ""}" loading="lazy">`;
  }
  return `<div class="arrivage-card__swatch">photo à venir</div>`;
}
...
function renderPieceDetail(el, p) {
  ...
  const mediaItems = [];
  if (p.video) mediaItems.push({ type: "video", url: p.video });
  photos.forEach((url) => mediaItems.push({ type: "image", url }));

  const mainMediaHtml = mediaItems.length
    ? mediaItems[0].type === "video"
      ? `<video id="piece-main-media" src="${mediaItems[0].url}" controls muted loop playsinline preload="metadata"></video>`
      : `<img id="piece-main-media" src="${mediaItems[0].url}" alt="${p.nom}">`
    : `<div class="piece__placeholder" id="piece-main-media">photo à venir</div>`;

  const thumbsHtml = mediaItems.length > 1
    ? `<div class="piece__thumbs">
        ${mediaItems
          .map(
            (m, i) => `<button type="button" class="piece__thumb" data-i="${i}">
      ...
      btn.addEventListener("click", () => {
        ...
        const newMedia =
          item.type === "video"
            ...
            : `<img id="piece-main-media" src="${item.url}" alt="${p.nom}">`;
        document.getElementById("piece-main-media").outerHTML = newMedia;
      });
    });
  }
